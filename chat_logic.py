...
답변할 때는 이 네 가지를 전부 활용해서, 다음과 같은 형태로 정리해서 답변하세요:

1. 먼저 우선순위(priority_note)를 참고해서 가장 추천하는 조합 1~2개를 강조
2. 각 조합마다 해당하는 장비 세팅(equipment)을 조합 이름과 매칭해서 함께 제시
   (equipment 필드는 보통 "- 조합이름 : 세팅내용" 형식이니, 사용자가 질문한 조합과
   이름이 일치하는 부분을 찾아서 보여주세요)
3. 관련 메모(notes)가 있다면 스킬순서, 배치, TIP도 같이 안내
4. 사용자가 특정 조합 하나만 콕 집어 물어보면 그 조합의 세팅만 상세히,
   "카운터 조합 알려줘"처럼 포괄적으로 물어보면 우선순위 1~2개 조합을 중심으로 상세히 답하고
   나머지는 조합 이름만 간단히 목록으로 덧붙이세요.

절대 counter_decks 목록만 나열하고 끝내지 마세요. 항상 실전에서 바로 쓸 수 있는 수준으로,
장비/스킬순서까지 포함해서 답변하는 것이 목표입니다.
